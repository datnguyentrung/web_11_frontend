...
export const createPoomsaeHistoryForElimination = async (
  competitorList: string[]
) => {
  try {
    console.log(
      "Creating poomsae history for elimination with competitors:",
      competitorList
    );
    const response = await axiosInstance.post(
      endpoints.poomsaeHistory.createForElimination,
      competitorList
    );
...
export const checkModePoomsaeHistoryExistence = async (
  ...
  try {
    ...
    console.error(
      "Error checking existence of poomsae history by filter:",
      error
    );
    throw error;
  }
...
export const createPoomsaeEliminationWinner = async (winnerId: string) => {
  ...
  try {
    const response = await axiosInstance.post(
      endpoints.poomsaeHistory.createWinnerForElimination(winnerId)
    );
    return response.data.data;
  } catch (error) {
...
export const deletePoomsaeHistoryForElimination = async (id: string) => {
  try {
    const response = await axiosInstance.delete(
      `${endpoints.poomsaeHistory.createForElimination}/${id}`
    );
    return response.data.data;
  } catch (error) {
...
export const filterPoomsaeHistories = async (
  idTournament: string,
  idCombination: string,
